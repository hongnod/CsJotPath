...
int CursorPos::m_nTitleHeight = HEADER_H;
...
void CursorPos::recalculate(const QPointF &gMousePos, const QRect &frameRect)
{
...
void WidgetAction::updateCursorShape(const QPointF &gMousePos)
{
...
void WidgetAction::resizeWidget(const QPointF &gMousePos)
{
...
void WidgetAction::moveWidget(const QPointF& gMousePos)
{
    ptrWidget->move(gMousePos.toPoint() - ptInWidgetPos.toPoint());
}
    ...
    if (isLeftButtonPressed)
    {
        ...
        else if (ptrHelperPriv->isWidgetMovable && isLeftButtonPressedTitle)
        {
            ...
            else
            {
                ...
                {
                    QRect frameRect = ptrWidget->frameGeometry();
                    ptInWidgetPos.setX(static_cast<int>(frameRect.width()*xfactor));
                    moveWidget(event->globalPos());
                    xfactor = 1.0;
                }
            }
...
void FramelessHelper::setBorderWidth(uint width)
{
    if (width > 0)
    {
        CursorPos::m_nBorderWidth = static_cast<int>(width);
    }
}
...
void FramelessHelper::setTitleHeight(uint height)
{
    if (height > 0)
    {
        CursorPos::m_nTitleHeight = static_cast<int>(height);
    }
}
...
{
    return static_cast<uint>(CursorPos::m_nBorderWidth);
}

uint FramelessHelper::titleHeight()
{
    return static_cast<uint>(CursorPos::m_nTitleHeight);
}
