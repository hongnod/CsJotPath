...
// this code is copied from http://blog.csdn.net/liang19890820/article/details/50557240, many thanks to the writer
#include <QObject>
#include <QMouseEvent>
#include <QHoverEvent>
...
public:
    explicit CursorPos();
    void reset();
    void recalculate(const QPointF &globalMousePos, const QRect &frameRect);
...
private:
    // 更新鼠标样式
    void updateCursorShape(const QPointF &gMousePos);
    // 重置窗体大小
    void resizeWidget(const QPointF &gMousePos);
    // 移动窗体
    void moveWidget(const QPointF &gMousePos);
    // 处理鼠标按下
