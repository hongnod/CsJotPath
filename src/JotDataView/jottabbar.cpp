...
JotTabBar::JotTabBar(QWidget *parent)
  : QTabBar(parent)
  ...
  , tabFixed_(false)
{
 ...
 // SetTextAlign()
  ...
  connect(this, &JotTabBar::customContextMenuRequested,
          this, &JotTabBar::showContextMenuJotTabBar);

  installEventFilter(this);
}

bool JotTabBar::eventFilter(QObject *obj, QEvent *event)
{
...
void JotTabBar::showContextMenuJotTabBar(const QPoint &pos)
{
...
void JotTabBar::slotCloseOtherTabs()
{
  int index = indexClickedTab_;
  int curIndex = -1;
  if (index == -1) index = currentIndex();
...
void JotTabBar::slotCloseAllTab()
{
  closingTabState_ = CloseTabOtherIndex;
  for (int i = count()-1; i > 0; i--) {
...
void JotTabBar::slotNextTab()
{
  setCurrentIndex(currentIndex()+1);
}

void JotTabBar::slotPrevTab()
{
  setCurrentIndex(currentIndex()-1);
}
