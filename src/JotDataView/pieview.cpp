    ...
    : QAbstractItemView(parent)
{
    ...
    if(nullptr == hdr) {
        hdr = new QHeaderView(Qt::Horizontal,this);
        hdr->setVisible(true);
    }
connect(hdr, SIGNAL(geometriesChanged()), this, SLOT(updateGeometries()));
}
                          ...
                          const QVector<int> &roles)
{
    ...
    //{
    //    qDebug("if (!roles.contains(Qt::DisplayRole))");
    //    return;
    //}
    validItems = 0;
    totalValue = 0.0;
    qDebug("run dataChanged!");
    ...
    for (int row = 0; row < model()->rowCount(rootIndex()); ++row) {
        ...
        if (value > 0.0) {
            ...
            if (sliceIndex == index) {
                ...
                return QRegion(slicePath.toFillPolygon().toPolygon());//这里有bug
            }
...
void PieView::paintEvent(QPaintEvent *event)
{
    ...
    QPen foreground(/*option.palette.color(QPalette::WindowText)*/Qt::red);
    ...
    painter.drawEllipse(0, hdr->height(), pieSize, pieSize);
    double startAngle = 0.0;
    int row;
    ...
    for (row = 0; row < model()->rowCount(rootIndex()); ++row) {
        ...
        if (value > 0.0) {
            ...
            painter.drawPie(0, hdr->height(), pieSize, pieSize, int(startAngle*16), int(angle*16));
...
void PieView::scrollContentsBy(int dx, int dy)
{
    ...
    hdr->setOffset(horizontalScrollBar()->value()); //让header一起滚动
}
...
void PieView::setSelection(const QRect &rect, QItemSelectionModel::SelectionFlags command)
{
    ...
    for (int row = 0; row < rows; ++row) {
        for (int column = 0; column < columns; ++column) {
            ...
            QRegion region = itemRegion(index);  //这里调用出问题
...
void PieView::updateGeometries()
{
    ...
    int height = 0;
    if (!hdr->isHidden()) {
        height = qMax(hdr->minimumHeight(), hdr->sizeHint().height());
        height = qMin(height, hdr->maximumHeight());
    }
    setViewportMargins(/*width*/0, height, 0, 0); //设置viewport的留边，以便放置Headerview
    QRect vg = viewport()->geometry();
    int horizontalTop = vg.top() - height;
    hdr->setGeometry(vg.left(),horizontalTop, vg.width(),height);

    horizontalScrollBar()->setPageStep(viewport()->width());
    horizontalScrollBar()->setRange(0, qMax(0, 2 * totalSize - viewport()->width()));
    verticalScrollBar()->setPageStep(viewport()->height()+height);
    verticalScrollBar()->setRange(height, qMax(0, totalSize - viewport()->height()));
...
void PieView::setModel(QAbstractItemModel * model)
{
